...
static const PROGMEM char webPage[] = "<!DOCTYPE html>" //Главнаявеб страница
  ...
  "  <a href= \"/i2c_scan.txt\">Сканирование I2C шины</a><br>\n"
  "  <a href= \"/serial_in.txt\">Просмотр сырых данных от подченённого МК</a><br>\n"
  "  <a href= \"/debug\">Просмотр данных отладки от подченённого МК</a><br>\n"
  "  <a href= \"/parse.txt\">Разбор сырых данных от подченённого МК</a>\n"
...
bool NAROD_data_send(char *str,short int size) { //Отправка данных на народны мониторинг
  ...
  sprintf(str,
		  ...
		  "#FHED#%d#Free ram in byte\n", mac, hostname_load, fw_ver/100, (fw_ver%100)/10, fw_ver%10, WiFi.RSSI(), ESP.getFreeHeap());
  if(data_rec == true) { 
  sprintf(str,
		  //"%s#MVC#%.2f\n"
		  "%s#MTMP#%.2f\n", str, mc_temp);
